...
!!! info
    The style guide is always a work in progress, and not all MNIST code
    follows the rules. When modifying MNIST, please fix the style violations
...
MNIST uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/MNIST.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the MNIST directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to MNIST have
